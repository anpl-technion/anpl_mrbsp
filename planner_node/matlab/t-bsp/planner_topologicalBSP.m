...
matlab_pub      = robotics.ros.Publisher (node_planner, m_matlab_service_response_topic,'std_msgs/UInt32');

session_num = 1;
save_results = false;
if has(rosparam, '/config_setup/log_folder') % output results go here
    base_dir = [rosparam('get','/config_setup/log_folder') '/t-bsp/'];
else
...
%     % switch to mpc after a prior is sufficiently large
%     if session_num == 5 % reached 4. goal
%         rosparam('set','/Robot_A/state_machine_default/state_switching_conditions/mpc',true); % switch to mpc
%     end
