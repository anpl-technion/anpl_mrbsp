    ...
    CollisionDetectionBase()
{
    ...
    nh_private.param("stop_for_other_robot", m_stop_for_other_robot, false);
    nh_private.param("stop_for_other_robot_radius", m_min_dist_other_robot, 0.0);
    int NUM_ROBOTS;
    nh_private.param("num_of_robots", NUM_ROBOTS, 2);
    m_current_pose.resize(NUM_ROBOTS);

    if (m_stop_for_other_robot)
    {
        /// subscribe to odometry topics of other robots
        for (int i = 0; i < NUM_ROBOTS; ++i)
        {
            std::string robot_id(sizeof(b_m_robot_id), 'A'+i);
            std::string robot_odom = "/Robot_" + robot_id + "/odometry";
            m_stop_sub.push_back(b_m_collision_detector_node.subscribe(robot_odom, 1,
                                                                     &CollisionDetectionOctomap::stoppingCallback, this));
            b_m_logger_msg << "CD: Subscribe to odometry messagess from: " << robot_odom;
            ANPL::anplLogMessage(b_m_p_cd_logger, info, 0, b_m_logger_msg);
        }
    }
...
void CollisionDetectionOctomap::stoppingCallback(const nav_msgs::OdometryConstPtr& pose_msg) {
    std::string robot_namespace(pose_msg->child_frame_id);
    char other_robot_id = robot_namespace[robot_namespace.size() - 1]; // to get the X in "Robot_X" string
    int other_robot_num = other_robot_id-'A';
    int this_robot_num = b_m_robot_id-'A';
    m_current_pose[other_robot_num] = (pose_msg->pose).pose;

    if ((int)b_m_robot_id <= (int)other_robot_id) {
    // if Robot_A and Robot_B are too close, A will ignore
        return;
    }

    // and B will stop till A will pass him
    gtsam::Pose3 my_current_pose = Conversion<gtsam::Pose3>::as(m_current_pose[this_robot_num]);
    gtsam::Pose2 my_current_pose_2D = gtsam::Pose2(my_current_pose.x(), my_current_pose.y(), my_current_pose.rotation().yaw());

	//# 2d option
    gtsam::Pose3 other_current_pose = Conversion<gtsam::Pose3>::as(m_current_pose[other_robot_num]);
    gtsam::Pose2 other_current_pose_2D = gtsam::Pose2(other_current_pose.x(), other_current_pose.y(), other_current_pose.rotation().yaw());

    std_msgs::Bool collision_with_other_msg;
    if (my_current_pose_2D.range(other_current_pose_2D) < m_min_dist_other_robot) {
        collision_with_other_msg.data = true;
    }
    else {
        collision_with_other_msg.data = false;
    }
    b_m_emergency_stop_pub.publish(collision_with_other_msg);
}
