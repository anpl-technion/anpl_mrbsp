...
#include "collision_detection_node/collision_detection_octomap.h"
    ...
    CollisionDetectionBase()
{
    ...
    if(!nh_private.hasParam("/config_setup/log_folder")) {
        ROS_WARN("Unable to find log folder...");
        ros::Duration(1.0).sleep();
        if(nh_private.hasParam("/config_setup/log_folder")) {
            nh_private.getParam("/config_setup/log_folder", b_m_path_to_log_folder);
        }
        else {
