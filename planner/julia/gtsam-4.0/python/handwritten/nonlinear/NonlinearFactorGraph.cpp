...
NonlinearFactorGraph deserializeGraph(const std::string& str) {
    NonlinearFactorGraph graph = NonlinearFactorGraph();
    deserialize(str, graph);
    return graph;
}
...
void exportNonlinearFactorGraph(){
  ...
  def("serializeGraph", serializeGraph);
  def("deserializeGraph", deserializeGraph);
